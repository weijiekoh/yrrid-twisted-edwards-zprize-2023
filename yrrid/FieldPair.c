...
void fieldPairMul(FieldPair_t* r, FieldPair_t* a, FieldPair_t* b) {
  ...
  // Annotations by weijiekoh (wj)

  // wj: this assigns the limbs of `b` to bd as doubles.
  // Recall that I2D(), defined in SIMD.h, casts an I128 into a F128.
  for(int i=0;i<5;i++)
    bd[i]=I2D(b->v[i]);

  // wj: these values were probably generated by a "make_initial()" function.
  // See Emmart's 2018 paper "Faster Modular Exponentiation using Double
  // Precision Floating Point Arithmetic on the GPU" (EZW18)
  ...
  // wj: Constants.
  // wj: I128 c0 = 2^51 - 1. It is used as a bitmask to extract the lower 51 bits of a value.
  c0=i64x2_splat(0x7FFFFFFFFFFFFL);

  // wj: I128 c1 = 2^52 as an 64-bit value (0x433 - 1023 = the exponent of 52)
  // In binary, it is 100001100110000000000000000000000000000000000000000000000000000
  // When added to a 52-bit (or 51-bit) value, it converts it to a floating point with exponent 52
  c1=i64x2_splat(0x4330000000000000L);

  // wj: F128 c2 = I2D(2^52) as a floating-point value (0x433 - 1023 = the exponent of 52)
  c2=I2D(c1);

  // wj: F128 c3 = 2^103 as a floating-point value (0x466 - 1023 = the exponent of 103)
  c3=I2D(i64x2_splat(0x4660000000000000L));

  // wj: F128 c4 = 2^103 + 3 * 2^51 as a floating-point value
  c4=I2D(i64x2_splat(0x4660000000000003L));

  // wj: p is the order of the base field of the Edwards BLS12 curve, in
  // little-endian form.
  ...
  for(int i=0;i<5;i++) {
    term=I2D(a->v[i]);
    for(int j=0;j<5;j++)
      // wj: FMA: lh[j] = a[i] * b[j] + 2^103
      // This isolates the high bits.
      lh[j]=f64x2_fma(term, bd[j], c3);
    for(int j=0;j<5;j++)
      // wj: sum[j + 1] += lh[j]
      // Accumulates the high term in sum[j+1]
      // Note that sum is already initialized with some magic numbers
      sum[j+1]=i64x2_add(sum[j+1], D2I(lh[j]));
    for(int j=0;j<5;j++)
      // wj: lh[j] = 2^103+3*2^51 - lh[j]
      // This computes sub = c4 - hi, where lh[j] is used to store sub
      lh[j]=f64x2_sub(c4, lh[j]);
    for(int j=0;j<5;j++)
      // wj: FMA: lh[j] = a[i] * b[j] + sub
      // This isolates the low bits.
      lh[j]=f64x2_fma(term, bd[j], lh[j]);
    for(int j=0;j<5;j++)
      // wj: sum[j] += lh[j]
      // Accumulates the low term in sum[j]
      sum[j]=i64x2_add(sum[j], D2I(lh[j]));

    // TODO: descrbe what sum contains now

    // In binary, 0x17FFFFFFFFFFFL is:
    // 0 000000000000 001011111111111111111111111111111111111111111111111
    ...
    // wj: F128 c2 = I2D(2^52) as a floating-point value (0x433 - 1023 = the exponent of 52)
    //
    // "cast" the lower 51 bits of q0 and 1 to be an f64 with exponent 0 (?)
    // This is done by masking the lower 51 bits, placing those bits in the
    // mantissa of an f64 with exponent 52 (c1), and then subtracting 2^52 in
    // the floating-point domain, leaving only the lower 51 bits. But could
    // this be made more efficient by masking and then adding to some value
    // encoded with bits with the desired exponent?
    term=f64x2_sub(
            I2D(
                i64x2_add(
                    i64x2_and(
    ...
    for(int j=0;j<5;j++)
      // wj: Get the high bits of q{0, 1} * p[j]
      // lh[j] = q{0, 1} * p[j] + 2^103
      lh[j]=f64x2_fma(term, p[j], c3);
    for(int j=0;j<5;j++)
      // wj: accumulate the above into sum
      // sum[j+1] += to_u64(lh[j])
      sum[j+1]=i64x2_add(sum[j+1], D2I(lh[j]));
    for(int j=0;j<5;j++)
      // wj: F128 c4 = 2^103 + 3 * 2^51 as a floating-point value
      // i.e. computes sub = c4 - high bits
      // lh[j] = c4 - (q{0, 1} * p[j] + 2^103)
      lh[j]=f64x2_sub(c4, lh[j]);
    for(int j=0;j<5;j++)
      // wj: Get the low bits of q{0, 1} * p[j]
    ...
    // wj: sum[0] = sum[1] + (sum[0] >> 51)
    // Also seen in EZW18, figure 9
    sum[0]=i64x2_add(sum[1], i64x2_shr(sum[0], 51));

    // wj: sum[1-3] = sum[2-4] + lh[2-4]
    for(int j=1;j<4;j++)
  ...
  // wj: Assign sum to the output r
  for(int i=0;i<5;i++)
    r->v[i]=sum[i];
}

// wj: fieldPairConvertMul is only used once in EC.c for
// pointPairXYTZ_accumulateXYT() (which implements addition-madd-2008-hwcd-4)
